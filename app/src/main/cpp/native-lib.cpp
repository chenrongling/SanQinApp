...
jint Java_com_gotop_sanqinapp_FFmpegHandle_setCallback(JNIEnv *env, jobject instance,
 ...
 */
jstring Java_com_gotop_sanqinapp_FFmpegHandle_getAvcodecConfiguration(JNIEnv *env,
                                                                      jobject instance) {
    char info[10000] = {0};
 ...
 */
JNIEXPORT jint JNICALL Java_com_gotop_sanqinapp_FFmpegHandle_pushRtmpFile(JNIEnv *env, jobject instance,
                                                                          jstring path_) {
    ...
    //输出的地址
    const char *outUrl = "rtmp://172.27.57.14:1935/live/test";
    ...
    try {
        ...
        //创建输出上下文
        ret = avformat_alloc_output_context2(&octx, NULL, "mp4", outUrl);
        if (ret < 0) {
            avError(ret);
            throw ret;
        }
 ...
 * Signature: (Ljava/lang/String;)I
 */
JNIEXPORT jint JNICALL Java_com_gotop_sanqinapp_FFmpegHandle_initVideo(JNIEnv *env, jobject instance,
                                                                       jstring url_) {
 ...
 */
int64_t startTime = 0;
jint Java_com_gotop_sanqinapp_FFmpegHandle_onFrameCallback(JNIEnv *env, jobject instance,
                                                           jbyteArray buffer_) {
//    startTime = av_gettime();
    ...
    int picture_size = av_image_get_buffer_size(pCodecCtx->pix_fmt
            , pCodecCtx->width,
                                                pCodecCtx->height, 1);
...
//        //将U数据存到第二个平面
    ...
    //Y
//    pFrameYUV->data[0] = (uint8_t *)in;
    //U
//    pFrameYUV->data[1] = (uint8_t *)in + y_length;
    //V
//    pFrameYUV->data[2] = (uint8_t *)in + y_length * 5 / 4;
 ...
 * Signature: ()I
 */
jint Java_com_gotop_sanqinapp_FFmpegHandle_close(JNIEnv *env, jobject instance) {
    if (video_st)
